<div class="container">
            ...
            <tbody>
                ...
                @forelse($announcements as $announcement)
                    <tr class="py-2" wire:key="announcement-{{ $announcement->id }}">
                        ...
                        <td>
                            <a href="{{ asset('storage/' . $announcement->announcement_asset) }}"
                                class="bi bi-file-earmark-fill text-primary text-decoration-underline" download></a>
                        </td>
            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h1 class="modal-title fs-5" id="viewAnnouncementModal{{ $announcement->id }}Label">Tazama
                            Tangazo</h1>
                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            ...
                            <input type="text" value="{{ $announcement->announcement_type }}"
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="description" id="description" class="form-control w-100 fw-bold"
                                style="max-height: 90px; min-height: 60px; height: 70px;" readonly>{{ $announcement->description }}</textarea>
                        </div>

                        <div class="mb-3">
                            <label for="name" class="form-label">Faili La Tangazo (Jina la Faili)</label>
                            <div class="p-2 rounded d-flex align-items-center justify-content-start w-100"
                                style="background-color: #00000020;">
                                <a href="{{ asset('storage/' . $announcement->announcement_asset) }}"
                                    class="text-decoration-none text-dark" download> <i
                                        class="bi bi-file-earmark-fill text-black mx-2"></i></a>
                                <div>{{ Str::limit($announcement->announcement_asset, 50, '...') }}</div>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="text"
                                        value="{{ $announcement->user->name . ' (' . Str::title($announcement->user->user_type) . ')' }}"
                                        class="form-control fw-bold" readonly>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="text" value="{{ $announcement->created_at }}"
                                        class="form-control fw-bold" readonly>
                                </div>
                            </div>
                        </div>
    ...
    @foreach ($announcements as $announcement)
        <div class="modal fade" id="editAnnouncementModal{{ $announcement->id }}" tabindex="-1"
            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <form action="{{ route('admin.edit.announcement') }}" method="POST" x-data="{ loading: false }"
                        @submit.prevent="loading = true; $el.submit()" enctype="multipart/form-data">
                        @csrf
                        <div class="modal-header">
                            <h1 class="modal-title fs-5" id="editAnnouncementModal{{ $announcement->id }}Label">
                                Hariri Tangazo</h1>
                            <button type="button" class="btn-close" data-bs-dismiss="modal"
                                aria-label="Close"></button>
                        </div>
                        ...
                        <div class="modal-body">
                            ...
                            {{-- <div class="mb-3">
                                ...
                                @if ($value)
                                    <div class="p-2 d-flex align-items-center justify-content-start rounded w-100" style="background-color: #00000020">
                                        ...
                                        <button type="button" class="btn bi bi-pen-fill text-primary me-2" onclick="document.getElementById('fileInputHidden').click();"></button>
                                        
                                        <!-- Hidden file input -->
                                        <input type="file" name="announcement_asset" id="fileInputHidden" class="d-none">
                            
                                        <i class="bi bi-file-earmark-fill text-dark mx-3"></i> 
                                        {{ Str::limit($announcement->announcement_asset, 20, '...') }}
                                    </div>
                                @else
                                    <input type="file" name="announcement_asset" id="announcement_asset" class="form-control" value="{{ old('announcement_asset', asset('storage/' . $announcement->announcement_asset)) }}">
                                @endif
                            
                                @error('announcement_asset')  
                            ...
                            <div class="mb-3">
                                ...
                                {{-- OLD FILE DISPLAY --}}
                                <div id="oldFileDisplay"
                                    class="p-2 d-flex align-items-center justify-content-start rounded w-100"
                                    style="background-color: #00000020; {{ $announcement->announcement_asset ? '' : 'display: none;' }}">
                                    
                                    <button type="button" class="btn bi bi-pen-fill text-primary me-2"
                                        onclick="triggerFileInput()"></button>
                            
                                    <i class="bi bi-file-earmark-fill text-dark mx-3"></i>
                            
                                    <span id="fileName">
                                        {{ Str::limit(basename($announcement->announcement_asset), 50, '...') }}
                                    </span>
                                </div>
                            
                                {{-- HIDDEN FILE INPUT --}}
                                <input type="file" name="announcement_asset" id="announcement_asset"
                                    class="form-control mt-2" style="display: none;" onchange="handleFileChange(event)">
                            
                                {{-- Send old file value if no change --}}
                                <input type="hidden" name="old_announcement_asset" value="{{ $announcement->announcement_asset }}">
                            ...
                            <div class="mb-3">
                                ...
                                @php
                                    $types = [
                                        'Ibada ya Kwanza (Jumapili)',
                                        'Ibada ya Pili (Jumapili)',
                                        'Ibada ya Tatu (Jumapili)',
                                        'Ibada ya Nne (Jumapili)',
                                        'Ibada ya Kawaida',
                                        'Tanzia',
                                        'Ndoa',
                                    ...
                                    $selected = old('announcement_type', $announcement->announcement_type);
                                @endphp
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="description" id="description" placeholder="Ingiza maelezo" class="form-control w-100"
                                    style="max-height: 100px; min-height: 50px; height: 70px;">{{ old('description', $announcement->description) }}</textarea>
                                @error('description')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header border-o">
                        <h1 class="modal-title fs-5" id="deleteAnnouncementModal{{ $announcement->id }}Label">Futa
                            Kikundi</h1>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"
                            aria-label="Close"></button>
                    </div>
    ...
    <!-- Modal - ADMIN DELETE SELECTED ANNOUNCMENTS -->
    <div class="modal fade" id="deleteSelectedAnnouncementModal" tabindex="-1"
        aria-labelledby="deleteSelectedAnnouncementModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header border-o">
    ...
    {{-- <script>
        function triggerFileInput() {
            document.getElementById('announcement_asset').click();
        }
    ...
    <script>
        ...
        function handleFileChange(event) {
            const fileInput = document.getElementById('announcement_asset');
            const fileDisplay = document.getElementById('oldFileDisplay');
    
            if (event.target.files.length > 0) {
                // Hide old file display
