...
class BaptismUpdateParentsCheckId extends Component
{
    ...
    public function mount($fatherId, $motherId)
    {
        ...
        $this->father = Member::find($fatherId);
        $this->mother = Member::find($motherId);

        if ($this->father) {
            $this->inputFatherMemberId = $this->father->member_id;
        }

        if ($this->mother) {
            $this->inputMotherMemberId = $this->mother->member_id;
        }
    }
